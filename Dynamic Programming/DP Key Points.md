...
| 状态来源         | `dp[i][j] = max(dp[i - 1][j], dp[i - 1][j - w[i]] + v[i])`(当j>= w时) | `dp[i][j] = max(dp[i - 1][j], dp[i][j - w[i]] + v[i])`(当j>= w时) |   |   |   |   |   |   |   |
| 一维化实现        | 对容量j进行逆序枚举                                                          | 对容量j进行正序枚举                                                      |   |   |   |   |   |   |   |
| 状态转移方程（一维dp） | `dp[j] = max(dp[j], dp[j - w[i]] + v[i])`(逆序处理j)                    | `dp[j]= max(dp[j], dp[j - w[i]] +v[i])`(正序处理j)                  |   |   |   |   |   |   |   |
| 物理意义         | 逆序保证了dp[j- w[i]]是旧状态，防止被重复迭代                                        | 正序利用了dp[j- w[i]]是新状态，允许重复迭代                                     |   |   |   |   |   |   |   |
|              |                                                                     |                                                                 |   |   |   |   |   |   |   |
|              |                                                                     |                                                                 |   |   |   |   |   |   |   |
|              |                                                                     |                                                                 |   |   |   |   |   |   |   |
